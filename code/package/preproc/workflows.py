...
def workflow_l1analysis(cfg, mask, regressor, strategy):
    ...
    wf.connect(l1contrasts, 'spm_mat_file', datasink, f"{wf.name}.contrasts.@spm_mat_file")
    wf.connect(l1contrasts, 'spmT_images', datasink, f"{wf.name}.contrasts.@spmT_images")
    wf.connect(l1contrasts, 'con_images', datasink, f"{wf.name}.contrasts.@con_images")
